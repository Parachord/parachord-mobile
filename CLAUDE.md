...
- **Apple Music developer token (JWT) — minted at BUILD TIME from the `.p8` (#186).** The `AppConfig.appleMusicDeveloperToken` JWT (used by the shared `AppleMusicArtistProvider` gap-filler for catalog artist images) is an ES256 JWT with `exp` ≤ 6 months. It used to be hand-pasted into two independent files, which drifted and expired silently. It is now **generated by ONE signer for both platforms** — `buildSrc/src/main/kotlin/AppleMusicToken.kt`:
  - **Android:** `app/build.gradle.kts` calls it → `BuildConfig.APPLE_MUSIC_DEVELOPER_TOKEN` → AppConfig.
  - **iOS:** the "Embed Apple Music developer token" Xcode phase runs `iosApp/scripts/embed-apple-music-token.sh`, which asks Gradle (`:app:printAppleMusicToken`) and PlistBuddy-writes the result into the BUILT `Info.plist`. (An xcconfig can't be fed by a build phase — xcconfig values resolve before any phase runs.)

  **Config** (`local.properties`, or env vars in CI): `APPLE_MUSIC_P8_PATH`, `APPLE_MUSIC_KEY_ID`, `APPLE_MUSIC_TEAM_ID`. **Rotation is now `.p8`-only** — nothing to paste. Without the `.p8` the build **falls back** to the legacy static `APPLE_MUSIC_DEVELOPER_TOKEN` (Android `local.properties` / iOS `Secrets.xcconfig`), so contributors without the key still build.

  Two non-obvious invariants in the signer, both of which fail SILENTLY if broken:
  - **DER→JOSE conversion is mandatory.** Java's `SHA256withECDSA` emits ASN.1 DER; JWS ES256 requires the raw 64-byte `R‖S`. Emitting DER yields a well-formed JWT that Apple 401s at request time — no build error.
  - **The token is cached under `build/` and stable per-day.** ECDSA signing is non-deterministic, so regenerating every build would rewrite `BuildConfig` and force a **full recompile every build**. `dayFlooredNow()` stabilizes the claims; the cache stabilizes the signature.

  `AppleMusicArtistProvider.isAvailable()` now also checks `exp` (`DeveloperTokenExpiry`), and logs a warning within 14 days of expiry — previously an expired token passed `isNotBlank()` and every request 401'd invisibly. This is a separate token from the on-device *playback* token (the App Service token used by `ApplicationMusicPlayer` on iOS — see `iosApp/AGENTS.md`).
