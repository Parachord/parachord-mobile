...
### N-way Reconcile — One-Way-Mirror Scope + ListenBrainz Re-Export

The N-way multimaster reconcile (`SyncEngine.propagateReconcilePlaylist` / `runNwayPropagation`) merges a playlist's copies across providers and materializes the result back to every writable mirror. **Real-writes (`nway_propagate`) stay OFF until desktop #911 ports** — these rules describe the staged behavior plus the two divergences mobile leads on.

**Single-authority (one-way mirror) vs. the missingStreak gate (#277).** The streak gate (parity with desktop #922) escalates a materialized track's deletion only after `MISSING_STREAK_THRESHOLD = 2` consecutive complete-fetch omissions, so a single transient omission isn't a deletion. But it must NOT apply to a source whose churn is the point: a FOLLOWED Spotify Daily Brew rotating ~40 tracks/day would never drop its rotated-out tracks (reintroducing the accumulation the pull-source-authority fix closed), and a wrong drop on a followed source self-heals next cycle so the gate buys nothing there. So the reconcile splits authority:

```
immediateAuthority = authoritativeCopyId == "local" || mirrorOnly || !playlist.writable
```

- **SINGLE-AUTHORITY (one-way mirror) → drops IMMEDIATELY, no gate:** a hosted-XSPF source (`"local"`), a FOLLOWED (`writable=false`) provider source, or a user-flagged mirror-only playlist.
- **OWNED, non-mirror provider source → the streak gate applies** (a transient complete-fetch omission could lose a real user edit). The partial-fetch floor runs BEFORE either path, so a truncated fetch never manufactures a drop.

This **diverges from desktop #922** (which gates EVERY source) in the right direction — desktop has the identical algorithmic-playlist break. Mobile leads; parity tracked on #911/#922.

**`mirrorOnly` — per-playlist user flag for OWNED-but-dynamic playlists.** Some dynamic playlists are written through the user's own OAuth by a third-party app (e.g. a **SmarterPlaylists**-managed Daily Brew), so the provider reports them as user-owned — the writability split alone can't tell them from a hand-curated owned playlist. The flag forces the one-way path for them. It lives in **KvStore** (`sync_playlist_mirror_only_<id>`), NOT a `playlists` column — a pull refreshes `writable` (#269) and would clobber a column, but can't touch KvStore. Surfaced as a "Mirror only (one-way)" toggle in the per-playlist Sync sheet on BOTH platforms via `PlaylistSyncChannelManager.getMirrorOnlyState` → `MirrorOnlyState(effective, forced)`; shown ON + LOCKED for followed/hosted playlists (`forced = !writable || sourceUrl != null`), user-toggleable for owned.

**ListenBrainz playlists can re-export to Spotify / Apple Music — OPT-IN only.** A `listenbrainz-*` playlist (created/edited on ListenBrainz via Achordion, or auto-pulled from LB) used to be a push candidate for NOTHING — the `isPlaylistPushCandidate` accept-lists grew one flow at a time (everything mirrors INTO LB; `spotify-*` mirrors to AM) and "LB as a re-export source" was never built. It's now eligible for `spotify` + `applemusic`. **But opt-in only:** `autoMirrorsByDefault(playlist) = !id.startsWith("listenbrainz-")`, so a LB row NEVER auto-mirrors via a provider's default (`mode=ALL`) selection — the user opts a specific playlist in by toggling the service in its Sync sheet (writes the per-playlist channel override, the authoritative path that bypasses the gate). Without this, enabling Spotify sync (always on) would push the user's entire pulled LB library to Spotify at once + risk the duplicate-flood. The guard is applied at BOTH the push-candidate filter (`pushPlaylistsForProvider`) AND the `effectivePlaylistChannels` seed so chips and pushes stay consistent. Create/edit propagation rides the EXISTING directional push (not the gated N-way real-writes): the first toggle → legacy push creates the mirror + hydrates tracks (confidence-gated catalog search); later LB edits → the pull flags `locallyModified` → the push updates the mirror.

**Key files:** `shared/.../sync/SyncEngine.kt` (`propagateReconcilePlaylist` immediate-authority split; `pushPlaylistsForProvider` + `effectivePlaylistChannels` opt-in gate), `shared/.../sync/SyncSettings.kt` (`isPlaylistPushCandidate`, `autoMirrorsByDefault`), `shared/.../sync/PlaylistSyncChannelManager.kt` (`MirrorOnlyState` / `getMirrorOnlyState` / `setMirrorOnly`), `shared/.../settings/SettingsStore.kt` (`getPlaylistMirrorOnly` / `setPlaylistMirrorOnly`, KvStore), per-playlist Sync sheet UI (`PlaylistSyncChannelsSheet.kt` Android + `PlaylistDetailView.swift` iOS).
