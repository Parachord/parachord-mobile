...
**Status — N-way real writes are OFF.** Two flags gate it: `isNwayEnabled()` (turns on detection + shadow/dry-run) AND `isNwayPropagateEnabled()` (arms real writes). `isNwayPropagateEnabled` is a **dev-only toggle, default OFF, never surfaced in Settings**. It's off because a **June 22 2026 data-loss incident** — a stale baseline under partial coverage + cross-service identity drift collapsed a 6-track playlist to 3 — drove the reconciliation redesign the current code implements; real writes stay off pending a no-false-drop harness + on-device validation + desktop #911 parity. **Today the LEGACY directional push loop (`pushPlaylistsForProvider`) is the SOLE live maintainer of push mirrors** — it creates links, updates tracks, detaches dead mirrors, honors the channel-override allowlist. N-way runs only shadow/dry-run. (Dead-mirror reconcile, #6, is the one exception — it runs live, ungated.)
...
### Apple Music library auth — the stale-MUT silent-401 trap (June 2026, PR #311)

**"Apple Music only shows N playlists / playlists not populating" is almost always a STALE Music-User-Token, NOT a fetch/pagination/decode bug.** The diagnostic that isolates it: the AM **library** API (`/me/library/...` — playlists, songs, albums, artists) is the ONLY surface that needs the MUT; **catalog resolution and streaming do NOT** (catalog uses the dev-token JWT, playback uses the MusicKit App-Service token). So if AM **resolution + streaming work but library sync fails**, it's the MUT — don't chase the dev-token JWT or the fetch code. The MUT is acquired once via `MusicKitWebBridge.authorize()` and **never auto-refreshes**; it goes stale on OS-level Apple Music sign-out / token revocation.

**Why it was silent (the trap, now fixed):** `AppleMusicLibraryClient.applyAuth`/`ensureSuccessOrThrowTyped` throw `AppleMusicReauthRequiredException` on missing-MUT/401, but `SyncEngine.pullPlaylistsForProvider`'s `catch (e: Exception)` swallowed it as "0 remote playlists" → the DB kept whatever stale rows a prior good sync left (the "only 8" symptom; the mass-floor guard preserved the links so nothing was lost). The fix: `AppleMusicSyncProvider.fetchPlaylists` **rethrows** the reauth exception (doesn't swallow); `SyncEngine` sets `appleMusicReauthRequired` (a `StateFlow<Boolean>` in commonMain; **self-clears on the next successful AM library fetch**); the Home screen shows a **"Reconnect Apple Music" banner** (`AppleMusicReauthBanner`, mirrors `SpotifyReauthBanner`) whose CTA re-mints the MUT via `MusicKitWebBridge.authorize()` + `clearAppleMusicReauth()`. **User fix:** Settings → Plug-Ins → Apple Music → **Disconnect, then Connect**.

**Library DTO hardening (so one bad row can't nuke a page):** `AmPlaylist.attributes` + `AmPlaylistAttributes.name` are nullable + defaulted, `AmPlaylistListResponse.meta` added; `fetchPlaylists` has a **per-page decode guard** (a malformed page logs + breaks, keeping prior pages) + meta-aware pagination. (Desktop JS is schemaless so it tolerated bad rows; kotlinx threw the whole page.) **Key files:** `AppleMusicLibraryClient.kt` (DTOs), `AppleMusicSyncProvider.kt#fetchPlaylists`, `SyncEngine.kt` (`appleMusicReauthRequired`/`clearAppleMusicReauth`, `pullPlaylistsForProvider` catch), `ui/components/AppleMusicReauthBanner.kt`, `ui/screens/home/HomeScreen.kt`.

### Sync Settings UI + the unified channel-override picker (#303 / #311)

**The "Configure what syncs" per-provider picker is the TRANSPOSE of the per-playlist channel override — both read/write the SAME authoritative state (`PlaylistSyncChannelManager`), so they can't drift.** This mirrors desktop's `channelOverrides[playlist.id]`. Do NOT use a parallel `pull_playlists` allowlist for the picker. The picker's toggle is `PlaylistSyncChannelManager.setChannel(localId, providerId, enabled)` — **detach-on-disable, never delete** → no Done-time keep/remove prompt. `getProviderChannelPlaylists(providerId)` is the transpose (one provider → all playlists where it's an available channel + each one's enabled state). The picker ALSO **live-fetches** the provider's not-yet-imported native playlists (check-to-import, shown "Tap to import"); the live-fetch MUST dedup against **native local rows + linked mirror copies (`SyncEngine.linkedExternalIdsForProvider`) + same-name local rows**, or it fabricates **0-track duplicate rows** (`fetchPlaylists` returns `trackCount=0` — that's the tell). Origin chips reuse the Playlists-tab `PlaylistSourceChip` brand colors (Spotify `#1DB954` / AM `#FA243C` / LB `#EB743B`).

**Sync-settings layout (#303 redesign, merged #310):** Sync is its OWN tab before General; a status header with a global **Sync now** (disabled while syncing AND when no service is enabled), staged progress label (`"Syncing Apple Music albums… (N/M)"`; Spotify tracks = "Spotify Liked Songs"), "Last synced …" when idle; three CONSISTENT cards (Spotify/AM/LB: Switch + one "Configure what syncs" + a "what's synced" summary). **Toggle-ON opens the config sheet as an enable GATE** — sync enables only on Done, cancel reverts (`pendingEnableProviderId` + `enableProviderFromConfig`). **Enabling Spotify needs BOTH** `settingsStore.setEnabledSyncProviders(+spotify)` AND `saveSyncSettings(SyncSettings(enabled=true, provider="spotify", ...))` (the legacy flag the Spotify card toggle reads) — AM/LB only need `setEnabledSyncProviders`. **Symptom→cause:** picker "0-track dupes" = live-fetch not deduping (native+linked+same-name); picker "only shows imported" = no live-fetch or stale MUT; "mirror" chip on an unchecked row = a non-native push candidate wrongly labeled (show the ORIGIN, not "mirror"). **Key files:** `ui/screens/sync/{SyncViewModel,ProviderSyncConfigSheet}.kt`, `ui/screens/settings/SettingsScreen.kt`, `shared/.../sync/PlaylistSyncChannelManager.kt`.

**iOS port (in progress, branch `feat/ios-sync-settings`):** the Kotlin foundation is landed (`IosContainer` bridges `watchAppleMusicReauthRequired`/`clearAppleMusicReauth`/`getSyncCollectionsForProvider`/`getProviderPickerRows`); the SwiftUI (tab + header + cards + summary, unified picker sheet, toggle-gate + reauth banner) is the focused next pass — plan in `docs/plans/2026-06-30-ios-sync-settings-port.md` + `ios-sync-kickoff.md`.
...
13. **Don't put a `/*` sequence inside a KDoc/block comment — Kotlin block comments NEST.** Writing `/me/library/*` (or any `/*`) inside a `/** … */` opens a NESTED block comment; the doc's closing `*/` then closes the *nested* one, leaving the OUTER comment unclosed → `Syntax error: Unclosed comment` at the file's end, with cascading "unresolved reference"/"cannot find X in scope" errors for everything the file should define. Bit the iOS `AppleMusicReauthBanner` KDoc ("the AM library API (/me/library/*)"). Write `/me/library` or `…/library/` — never a literal `/*` in prose. (June 2026, #311.)
